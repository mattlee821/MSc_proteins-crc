...
data_SNP <- data_exposure %>%
  filter(SNP == "rs190287113") #SNP from SIGMOID MR
...
write.table(data_protein_window, "analysis/005_colocalisation_gene-proteins/sigmoid/Exposure_GREM1-gene_sigmoid_window_coloc.txt", sep="\t", row.names = FALSE, quote = FALSE, col.names = TRUE)
...
data_SNPlist <- data_protein_window %>%
  ...
  unique() %>%
  as.data.frame()
write.table(data_SNPlist, "analysis/005_colocalisation_gene-proteins/sigmoid/Exposure_GREM1-gene_sigmoid_SNPlist.txt", sep="\t", row.names = FALSE, quote = FALSE, col.names = TRUE)
