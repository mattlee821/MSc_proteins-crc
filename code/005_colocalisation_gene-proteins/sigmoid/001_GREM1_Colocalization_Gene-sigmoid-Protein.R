...
data_exposure <- fread("analysis/005_colocalisation_gene-proteins/sigmoid/Exposure_GREM1-gene_sigmoid_window_coloc.txt")
data_outcome <- fread("analysis/005_colocalisation_gene-proteins/sigmoid/Outcome_GREM1-protein_sigmoid_coloc.txt")
head(data_exposure)
head(data_outcome)

#Select SNP - Use SNP from MR GREM1 Gene expression (sigmoid) -CRC (rs190287113), exposure data = gene expression
SNP <- "rs190287113"  
...
write.table(table_coloc, "analysis/005_colocalisation_gene-proteins/sigmoid/001_genesigmoid-protein_table_coloc_1mb.txt", sep="\t", row.names = FALSE, quote = FALSE, col.names = TRUE)
...
tiff(filename = "analysis/005_colocalisation_gene-proteins/sigmoid/plot-sensitivity_sigmoid_1mb.tiff", width = 1000, height = 800, units = "px")
plot
dev.off()
