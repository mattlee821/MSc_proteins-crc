...
plot_data <- bind_rows(
  ...
  table_mr_crc_protein_exclusion %>%
    mutate(
      Section = "CRC - Plasma Protein",
      Analysis = paste("CRC - Plasma GREM1 (", Method, ")", sep = "")  # Method included in analysis name
    ) %>%
    select(Study = Exposure, Section, Analysis, Method, Estimate, SE = Std_Error, P_Value),
...
# Set custom order for the Method factor
plot_data$Method <- factor(plot_data$Method, levels = c("Inverse variance weighted", "MR Egger", "Weighted median", "Simple mode", "Weighted mode"))

# Set custom order for the Section factor
plot_data$Section <- factor(plot_data$Section, levels = c("Plasma Protein - CRC", "Gene Expression - CRC", "CRC - Plasma Protein"))

# Sort data based on Method to ensure correct order in the plot
plot_data <- plot_data %>%
  arrange(Section, Method)
...
ggforestplot::forestplot(
  ...
  theme(
    axis.text.y = element_text(size = 12, hjust = 0),  # Align y-axis text (analysis names) to the left
    strip.text = element_text(size = 12),  # Adjust facet label text size
    legend.position = "none",              # Hide legend for clarity
    axis.title.x = element_text(size = 12, hjust = 0.5),  # Adjust x-axis label size and alignment
