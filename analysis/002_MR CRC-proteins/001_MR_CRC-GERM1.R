...
shorten_names <- function(name) {
  gsub("fernandez-rozadilla_2022_PMID36539618;crc_combined_GECCO_allancestries;CRC", "CRC_GECCO", name)
}
...
table_mr_crc_protein <- data_mr_crc_protein %>%
  ...
  rename(Method = method, Estimate = b, Std_Error = se, P_Value = pval)
